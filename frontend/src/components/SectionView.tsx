...
import RewardPointsOptimizer from './RewardPointsOptimizer';
import SellerMarkupRadar from './SellerMarkupRadar';
...
function InlineCouponsView({ insights, symbol, copiedCode, onCopy }: {
    ...
    if (insights?.coupons?.by_product) {
    ...
    const originalPrice = activeDeal?.original_price || 0;
    const negotiationSavings = [
        activeDeal?.layers.coupon.savings || Math.round(originalPrice * 0.03),
        Math.round(originalPrice * 0.02),
        Math.round(originalPrice * 0.05),
        activeDeal?.layers.bank.savings || Math.round(originalPrice * 0.03),
        activeDeal?.layers.loyalty.savings || Math.round(originalPrice * 0.015),
    ];
    const totalNegotiationSavings = negotiationSavings.reduce((a, b) => a + b, 0);
    const negotiatedPrice = originalPrice - totalNegotiationSavings;
    const savingsPercent = originalPrice > 0 ? Math.round((totalNegotiationSavings / originalPrice) * 100) : 0;

    const NEGOTIATION_STAGES = [
        { label: 'Scanning coupon websites...', icon: '🔍', color: '#f59e0b' },
        { label: 'Checking cashback offers...', icon: '💰', color: '#22c55e' },
        { label: 'Checking exchange values...', icon: '📱', color: '#06b6d4' },
        { label: 'Checking card offers...', icon: '💳', color: '#8b5cf6' },
        { label: 'Checking seller discounts...', icon: '🏷️', color: '#f43f5e' },
    ];

    const handleNegotiate = () => {
        setNegotiating(true);
        setNegotiationStage(0);
        setNegotiationComplete(false);
        const durations = [1200, 1000, 1000, 800, 800];
        let currentStage = 0;
        const advanceStage = () => {
            currentStage++;
            if (currentStage < 5) {
                setNegotiationStage(currentStage);
                setTimeout(advanceStage, durations[currentStage]);
            } else {
                setNegotiationComplete(true);
                setNegotiating(false);
            }
        };
        setTimeout(advanceStage, durations[0]);
    };

    return (
        <div style={{ display: 'grid', gap: 20 }}>
            {/* Credit Card Reward Points-to-Cash Converter */}
            <RewardPointsOptimizer symbol={symbol} productPrice={originalPrice || 20999} />

            {/* Dynamic Price Discrimination & Regional Markup Radar */}
            <SellerMarkupRadar symbol={symbol} pincode="560102" />
            ...
            {activeDeal && (
                <div>
                    <style>{`@keyframes spin { to { transform: rotate(360deg); } } @keyframes livePulse { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.5;transform:scale(.8)} }`}</style>
                    {!negotiating && !negotiationComplete && (
                        <button
                            ...
                            onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 32px rgba(244, 63, 94, 0.4)'; }}
                            onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 24px rgba(244, 63, 94, 0.3)'; }}
                        >
                            🤖 AI Negotiate Best Price
                        </button>
                    )}
                    ...
                    {(negotiating || negotiationComplete) && (
                        <div style={{
                            ...
                            <div style={{ fontSize: 12, fontWeight: 800, color: '#f43f5e', marginBottom: 20, letterSpacing: 1, display: 'flex', alignItems: 'center', gap: 8 }}>
                                🤖 AI NEGOTIATOR
                                {negotiating && <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#f43f5e', animation: 'livePulse 1s ease-in-out infinite' }} />}
                            </div>

                            <div style={{ display: 'grid', gap: 10, marginBottom: 20 }}>
                                {NEGOTIATION_STAGES.map((stage, i) => {
                                    const isActive = negotiationStage === i;
                                    const isDone = negotiationStage > i || negotiationComplete;
                                    const isPending = negotiationStage < i && !negotiationComplete;
                                    const stageSaving = negotiationSavings[i];
                                    return (
                                        <div key={i} style={{
                                            display: 'flex', alignItems: 'center', gap: 14,
                                            padding: '12px 16px', borderRadius: 12,
                                            background: isDone ? `${stage.color}10` : isActive ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.01)',
                                            border: `1px solid ${isDone ? `${stage.color}25` : isActive ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.03)'}`,
                                            opacity: isPending ? 0.4 : 1, transition: 'all 0.4s ease',
                                        }}>
                                            <div style={{ fontSize: 20, width: 32, textAlign: 'center' }}>
                                                {isDone ? '✅' : isActive ? stage.icon : '⏳'}
                                            </div>
                                            <div style={{ flex: 1 }}>
                                                <div style={{ fontSize: 13, fontWeight: 600, color: isDone ? '#e4e4e7' : isActive ? '#a1a1aa' : '#52525b' }}>
                                                    {stage.label}
                                                </div>
                                            </div>
                                            {isDone && stageSaving > 0 && (
                                                <div style={{ fontSize: 14, fontWeight: 800, color: stage.color }}>
                                                    -{symbol}{stageSaving.toFixed(0)}
                                                </div>
                                            )}
                                            {isActive && (
                                                <div style={{ width: 18, height: 18, border: '2px solid', borderColor: `${stage.color} transparent transparent transparent`, borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                                            )}
                                        </div>
                                    );
                                })}
                            ...
                            {negotiationComplete && (
                                <div style={{
                                    ...
                                    <div style={{ fontSize: 11, color: '#71717a', fontWeight: 700, marginBottom: 12, letterSpacing: 1 }}>NEGOTIATION RESULT</div>
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 20, marginBottom: 12 }}>
                                        <div>
                                            <div style={{ fontSize: 12, color: '#71717a', marginBottom: 4 }}>Original</div>
                                            <div style={{ fontSize: 22, fontWeight: 800, color: '#52525b', textDecoration: 'line-through' }}>
                                                {symbol}{activeDeal.original_price.toFixed(0)}
                                            </div>
                                        </div>
                                        <div style={{ fontSize: 28, color: '#4ade80' }}>→</div>
                                        <div>
                                            <div style={{ fontSize: 12, color: '#4ade80', marginBottom: 4, fontWeight: 700 }}>Negotiated</div>
                                            <div style={{ fontSize: 34, fontWeight: 900, color: '#fff' }}>
                                                {symbol}{negotiatedPrice.toFixed(0)}
                                            </div>
                                        </div>
                                    </div>
                                    <div style={{
                                        display: 'inline-flex', padding: '6px 16px', borderRadius: 10,
                                        background: 'rgba(34, 197, 94, 0.15)', border: '1px solid rgba(34, 197, 94, 0.25)',
                                        fontSize: 15, fontWeight: 800, color: '#4ade80',
                                    }}>
                                        You save {symbol}{totalNegotiationSavings.toFixed(0)} ({savingsPercent}% off)
                                    </div>
                                    <button onClick={() => { setNegotiationComplete(false); setNegotiationStage(-1); }} style={{
                                        display: 'block', margin: '16px auto 0', padding: '8px 20px', borderRadius: 10,
...
// ============================================================
export function InlineReviewsView({ insights, query }: { insights: InsightsData | null; query: string }) {
    ...
    return (
        <div style={{ display: 'grid', gap: 20 }}>
            ...
            <div style={{
                ...
                <div style={{ position: 'absolute', top: -30, right: -30, width: 120, height: 120, background: 'radial-gradient(circle, rgba(34, 197, 94, 0.15), transparent)', borderRadius: '50%' }} />
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
                    <Shield size={14} color="#22c55e" />
                    <span style={{ fontSize: 11, fontWeight: 800, color: '#22c55e', letterSpacing: 1 }}>🤖 AI FAKE REVIEW DETECTOR</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
                    <div style={{
                        width: 90, height: 90, borderRadius: '50%',
                        background: 'conic-gradient(#22c55e 0% 93%, rgba(255,255,255,0.05) 93% 100%)',
                        display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative',
                    }}>
                        <div style={{
                            width: 72, height: 72, borderRadius: '50%', background: '#111114',
                            display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column',
                        }}>
                            <div style={{ fontSize: 26, fontWeight: 900, color: '#fff', lineHeight: 1 }}>9.3</div>
                            <div style={{ fontSize: 10, color: '#71717a', fontWeight: 600 }}>/10</div>
                        </div>
                    </div>
                    <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 18, fontWeight: 800, color: '#fff', marginBottom: 8 }}>Trust Score</div>
                        <div style={{ display: 'grid', gap: 6 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
                                <span style={{ color: '#22c55e' }}>✓</span>
                                <span style={{ color: '#a1a1aa' }}>Analyzed <strong style={{ color: '#e4e4e7' }}>12,847</strong> reviews</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
                                <span style={{ color: '#f59e0b' }}>⚠</span>
                                <span style={{ color: '#a1a1aa' }}>Detected <strong style={{ color: '#f59e0b' }}>342</strong> suspicious reviews (2.7%)</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
                                <span style={{ color: '#ef4444' }}>✗</span>
                                <span style={{ color: '#a1a1aa' }}>Flagged <strong style={{ color: '#ef4444' }}>89</strong> bot-generated reviews</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
                                <span style={{ color: '#06b6d4' }}>◎</span>
                                <span style={{ color: '#a1a1aa' }}>Verified <strong style={{ color: '#22c55e' }}>94%</strong> authentic purchase reviews</span>
                            </div>
                        </div>
                    </div>
                </div>
