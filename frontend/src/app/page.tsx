...
import ShoppingHealthScore from '@/components/ShoppingHealthScore';
import AIMarketplace from '@/components/AIMarketplace';
import BrowserExtensionDemo from '@/components/BrowserExtensionDemo';
import PurchaseSimulation from '@/components/PurchaseSimulation';
import LifePlanner from '@/components/LifePlanner';
import CommunityIntelligence from '@/components/CommunityIntelligence';
import FlashPools from '@/components/FlashPools';
import PredictivePriceOracle from '@/components/PredictivePriceOracle';
...
import JudgeDemoStory from '@/components/JudgeDemoStory';
import KeyboardShortcutsModal from '@/components/KeyboardShortcutsModal';
import MobileNav from '@/components/MobileNav';
import VisionTrustSuite from '@/components/VisionTrustSuite';
import WhatsAppAlertBot from '@/components/WhatsAppAlertBot';
import AutonomousAgentCheckout from '@/components/AutonomousAgentCheckout';
import RewardPointsOptimizer from '@/components/RewardPointsOptimizer';
import SellerMarkupRadar from '@/components/SellerMarkupRadar';
...
const SECTION_PAGES = new Set([
  'coupons', 'oracle', 'campus',
  'history', 'saved', 'cart', 'alerts', 'favourites', 'agent',
  'visual', 'alternatives', 'receipts', 'health_score',
  'marketplace', 'extension', 'simulation', 'life_planner', 'flash_pools', 'community', 'loyalty', 'insights', 'ml_studio', 'vision_trust', 'whatsapp_bot', 'auto_checkout', 'points_converter', 'markup_radar',
]);
...
export default function Home() {
  ...
  const [activeARProduct, setActiveARProduct] = useState<ProductResult | null>(null);
  ...
  const [explainModalContent, setExplainModalContent] = useState({ title: '', metricChange: '' });
  ...
  // Global Keyboard Shortcuts (⌘K, /, g h, g a, ?, Esc)
  useEffect(() => {
    let lastKey = '';
    let keyTimeout: any;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Chord sequence (G then H -> Home, G then A -> Analytics)
  ...
  // Sort product groups and internal products based on selected option
  const sortedProductGroups = useMemo(() => {
    if (!result?.product_groups) return [];

    // Clone groups and sort internal platform offers
    const groups = result.product_groups.map(g => {
      const sortedProducts = [...g.products].sort((a, b) => {
        const costA = a.price_breakdown?.total_landed_cost || 0;
        const costB = b.price_breakdown?.total_landed_cost || 0;
        if (sortBy === 'price_low') return costA - costB;
        if (sortBy === 'price_high') return costB - costA;
        if (sortBy === 'time_fast') return a.eta_minutes - b.eta_minutes;
        return 0;
      });
      return {
        ...g,
        products: sortedProducts,
        best_price: sortedProducts[0] || g.best_price,
        fastest_delivery: [...sortedProducts].sort((a, b) => a.eta_minutes - b.eta_minutes)[0] || g.fastest_delivery
      };
    });
    ...
    switch (sortBy) {
      case 'price_low':
        return groups.sort((a, b) =>
          (a.best_price.price_breakdown?.total_landed_cost || 0) - (b.best_price.price_breakdown?.total_landed_cost || 0)
        );
      case 'price_high':
        return groups.sort((a, b) =>
          (b.best_price.price_breakdown?.total_landed_cost || 0) - (a.best_price.price_breakdown?.total_landed_cost || 0)
        );
      case 'time_fast':
        return groups.sort((a, b) =>
    ...
    <div style={{
      ...
      <div style={{
        position: 'relative',
        marginLeft: sidebarExpanded ? 260 : 72,
          ...
          {isShowingSection ? (
              activeSection === 'agent' ? (
                  <ShoppingAgent
                      ...
                      country={country}
                  />
              ) : activeSection === 'vision_trust' ? (
                  <VisionTrustSuite
                      symbol={result?.currency_symbol || currentCountry.symbol}
                      pincode={postalCode}
                      onSearchTrigger={(q) => { setQuery(q); handleSearch(q); setActiveSection('home'); }}
                  />
              ) : activeSection === 'whatsapp_bot' ? (
                  <WhatsAppAlertBot
                      symbol={result?.currency_symbol || currentCountry.symbol}
                      pincode={postalCode}
                  />
              ) : activeSection === 'auto_checkout' ? (
                  <AutonomousAgentCheckout
                      symbol={result?.currency_symbol || currentCountry.symbol}
                      pincode={postalCode}
                  />
              ) : activeSection === 'points_converter' ? (
                  <RewardPointsOptimizer
                      symbol={result?.currency_symbol || currentCountry.symbol}
                  />
              ) : activeSection === 'markup_radar' ? (
                  <SellerMarkupRadar
                      symbol={result?.currency_symbol || currentCountry.symbol}
                      pincode={postalCode}
                  />
                  ...
                  <CommunityIntelligence
                      ...
                      query={query}
                  />
              ) : activeSection === 'oracle' || activeSection === 'ml_studio' ? (
                  <PredictivePriceOracle
                      symbol={result?.currency_symbol || currentCountry.symbol}
                      pincode={postalCode}
                  />
              ) : activeSection === 'loyalty' ? (
                  <LoyaltyDashboard />
              ) : activeSection === 'flash_pools' ? (
                  <FlashPools />
            ...
            <>
              ...
              <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32, gap: 12, flexWrap: 'wrap' }}>
                {/* Live Platform Pulse Pill */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 16px', borderRadius: 20, background: '#111114', border: '1px solid #27272a' }}>
                  ...
                  <span style={{ fontSize: 12, color: '#a1a1aa', fontWeight: 600 }}>
                    7 Live Stores: <strong style={{ color: '#fff' }}>Amazon • Flipkart • Croma • Blinkit • Zepto</strong>
                  </span>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <button
                    onClick={() => setIsDemoStoryOpen(true)}
                    style={{
                      display: 'flex', alignItems: 'center', gap: 8,
                      padding: '9px 16px', background: 'linear-gradient(135deg, rgba(168, 85, 247, 0.2), rgba(59, 130, 246, 0.2))',
                      border: '1px solid rgba(168, 85, 247, 0.45)', borderRadius: 12, cursor: 'pointer', color: '#c084fc',
                      fontWeight: 800, fontSize: 13, boxShadow: '0 0 18px rgba(168, 85, 247, 0.2)',
                      transition: 'all 0.2s ease'
                    }}
                    onMouseEnter={e => { e.currentTarget.style.transform = 'scale(1.03)'; e.currentTarget.style.borderColor = '#c084fc'; }}
                    onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.borderColor = 'rgba(168, 85, 247, 0.45)'; }}
                  >
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: 200, position: 'relative', display: 'flex', alignItems: 'center' }}>
                    <Search size={20} style={{ position: 'absolute', left: 18, color: '#52525b' }} />
                    <input
                      value={query}
                      ...
                      style={{
                        ...
                        padding: '18px 100px 18px 52px', color: '#fff', fontSize: 15, outline: 'none',
                      }}
                    />
                    <div style={{ position: 'absolute', right: 12, display: 'flex', gap: 8, alignItems: 'center' }}>
                        <button
                            ...
                            style={{
                                padding: '4px 8px', borderRadius: 6, background: '#1a1a1e', border: '1px solid #27272a',
                                color: '#a1a1aa', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4
                            }}
                        >
                            ⌘K
                        </button>
                        <button 
                            ...
                            style={{ 
                                ...
                                border: 'none', color: isListening ? '#ef4444' : '#71717a', cursor: 'pointer',
      ...
      {/* 3-Step Persona Onboarding Wizard */}
      <OnboardingWizard
        isOpen={isOnboardingOpen}
        onClose={() => setIsOnboardingOpen(false)}
        onComplete={({ goal, priorityInfo }) => {
          if (goal === 'Bulk Group Savings') {
            handleSectionChange('community');
          } else if (goal === 'Predict Price Drops' || priorityInfo.includes('Error Drops')) {
            handleSectionChange('oracle');
          } else if (priorityInfo.includes('Reviews')) {
      ...
      {/* Universal 'Why / Explain This' Modal */}
      <ExplainThisModal
        isOpen={isExplainModalOpen}
        onClose={() => setIsExplainModalOpen(false)}
        title={explainModalContent.title}
        metricChange={explainModalContent.metricChange}
        onViewData={() => handleSectionChange('oracle')}
      />

      {/* 7-Step Hackathon Judge Demo Story Pipeline */}
      <JudgeDemoStory
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '28px 32px', borderBottom: '1px solid #141416', display: 'flex', gap: 24, alignItems: 'flex-start' }}>
        {/* Product Group Image */}
        {(() => {
          const validImgProduct = group.products.find(p => p.image_url && p.image_url.startsWith('http'));
          const displayImg = validImgProduct?.image_url || best.image_url;
          return (
            <div style={{
              ...
              <div style={{
                display: (!displayImg || !displayImg.startsWith('http')) ? 'flex' : 'none',
                flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                width: '100%', height: '100%',
                background: 'linear-gradient(135deg, rgba(6, 182, 212, 0.15), rgba(99, 102, 241, 0.1))',
              }}>
                <Zap size={28} color="#06b6d4" />
                <span style={{ fontSize: 10, fontWeight: 700, color: '#a1a1aa', marginTop: 4 }}>
                  {PLATFORM_CONFIG[best.platform]?.name || 'LIVE STORE'}
                </span>
              </div>
            </div>
          );
