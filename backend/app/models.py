...
class ChatResponse(BaseModel):
    reply: str
    recommended_products: List[ProductResult] = []
...
class ForecastUpcomingSale(BaseModel):
    eventName: str
    dateRange: str
    projectedPrice: int
    dropPct: float
...
class ForecastChartData(BaseModel):
    day: str
    price: int
    type: str # 'historical' | 'predicted'
    minBound: Optional[int] = None
    maxBound: Optional[int] = None

class ForecastResponse(BaseModel):
    query: str
    productTitle: str
    currentPrice: int
    recommendedAction: str # 'BUY' | 'WAIT'
    confidence: int
    predictedLowestPrice: int
    predictedLowestDays: int
    mapeAccuracy: float
    historicalHigh90: int
    historicalLow90: int
    historicalAverage90: int
    reasons: List[str]
    upcomingSales: List[ForecastUpcomingSale]
    chartData: List[ForecastChartData]
