...
def calculate_carbon_footprint(products: List[ProductResult]) -> Dict:
    ...
    best_plat = min(platforms, key=lambda p: PLATFORM_CARBON_DATA.get(p, {"co2_kg": 9.9})["co2_kg"]) if platforms else PlatformType.AMAZON_IN
    best_plat_val = best_plat.value if hasattr(best_plat, 'value') else str(best_plat)
    
    return {
        "by_platform": by_platform,
        "total_avg_co2": round(avg_co2, 2),
        "best_platform": best_plat_val,
